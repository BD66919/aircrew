...
public class DataServiceImpl implements DataService {
    ...
    public List<Mp> compareAirMp() {
        ...
        for (Mp mp : mpList) {
            ...
            if (!tccAmend.toString().equals(mp.getTcc())) {
                if (result.getDate() != null) {
                    if(mp.getTcc()==null)
                        result.setTcc(tccAmend.toString() + " 人力：" + mp.getAirLine());
                    else
            ...
            if (!isAppear) {
                result.setDate(mp.getDate());
                result.setFlightNo(mp.getFlightNo());
                result.setTcc(mp.getTcc()+" 该条数据在人力数据中不存在");
                result.setEid(mp.getEid());
                result.setName(mp.getName());
        ...
        for (Mp mp:mpList
            ...
            String date = mp.getDate();
            for(String no:flightNo) {
                boolean appear = false;
                for (Air air : airList) {
                    if (!air.getDate().equals(date))
                        continue;
                ...
                if (!appear) {
                    mp.setTcc(mp.getTcc() + " 该条数据在飞行数据中不存在");
                    mpResultList.add(mp);
                }
            }
        }
